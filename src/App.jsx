    ...
    <div style={{ height: "100dvh", background: C.bg, color: C.text, fontFamily: "'Courier New', monospace", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "20px 16px 32px" }}>
        ...
        <div style={{ background: C.surface, border: `1px solid ${success ? C.green : error ? C.red : C.border}`, borderRadius: 12, padding: 18, marginBottom: 14, transition: "border-color 0.2s" }}>
          ...
          <button onClick={handleSave} disabled={!key.trim() || success}
            style={{ width: "100%", background: success ? C.green : key.trim() ? C.gold : C.faint, color: success || key.trim() ? "#000" : C.muted, border: "none", padding: "13px 0", fontFamily: "Bangers, cursive", fontSize: 15, letterSpacing: 3, cursor: key.trim() && !success ? "pointer" : "not-allowed", borderRadius: 8, transition: "all 0.2s" }}>
...
// ─── Settings Panel ───────────────────────────────────────────────────────────
function SettingsPanel({ activeApi, setActiveApi, keys, setKeys, targetLang, setTargetLang, fontStyle, setFontStyle, onClose }) {
...
export default function App() {
  ...
  const processFile = async file => {
    ...
    try {
      ...
      addLog(`🖼  Found ${imageFiles.length} pages · Translating to ${targetLang} via ${APIS[activeApi].name}`);
      const results = [];
      ...
      for (let i = 0; i < imageFiles.length; i++) {
        ...
        let bubbles = [];
        try {
          const r = await translatePage(b64, activeApi, apiKey, targetLang);
          bubbles = r.bubbles || [];
          addLog(`   ✅ ${bubbles.length} bubble${bubbles.length !== 1 ? "s" : ""}`);
        } catch (e) {
          if (e.message === "RATE_LIMIT") addLog("   ⏳ Rate limit — waiting 8s…");
        ...
        if (i < imageFiles.length - 1 && activeApi === "gemini") {
          await new Promise(r => setTimeout(r, 4200));
        }
      }

      setStatus("done");
      addLog(`✅ Done! ${imageFiles.length} pages → ${targetLang}`);
    } catch (e) {
