...
async function validateKey(apiKey) {
  // Google Gemini keys are always "AIza" + 35 chars = 39 total
  // Validate by format only — avoids 429 rate limit errors on the check itself
  if (!apiKey.startsWith("AIza")) {
    return { ok: false, error: 'Key should start with "AIza…" — copy it again from AI Studio.' };
  }
  if (apiKey.length < 35) {
    return { ok: false, error: "Key looks too short — make sure you copied the whole thing." };
  }
  if (apiKey.length > 45) {
    return { ok: false, error: "Key looks too long — make sure you only copied the key itself, nothing extra." };
  }
  // Format looks good — accept it. If it's actually wrong, the first translation
  // attempt will fail and show a clear error.
  return { ok: true };
}
